...
```cron
*/15 * * * * cd /Users/kmeyer/Github/the-council/02-vinted-watch && .venv/bin/python -m vinted_watch.cli --config config/config.yaml --favorite --csv state/matches.csv --db state/vinted_watch.db >> state/run.log 2>&1
```
